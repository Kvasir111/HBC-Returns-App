    ...
    export default {
        ...
        methods: {
            ...
            exportPDF() {
                ...
                doc = this.writeCustomerString(doc, fontSize);
                doc = this.writeEquipmentString(doc, fontSize);
                doc = this.writeReturnString(doc, fontSize);
                doc = this.writeDateTimeStamp(doc, fontSize);
                //saves doc
                doc.save(this.firstName + "_" + this.lastName + "_return.pdf");
            },
            writeCustomerString(doc, fontSize) {
                ...
                doc.setFontSize(fontSize);
                for (let i = 0, x = 72, y = 150 ; i < customerInformation.length ; i++){
                    doc.setFontType('bold');
                    doc.text(customerInformation[i].text, x, y);
                    doc.setFontType('normal');
                    let labelLength = doc.getStringUnitWidth(customerInformation[i].text) * fontSize;
                    doc.text(customerInformation[i].value, labelLength + x + 5 , y);
                    y = y + fontSize + 1; //increments x by text size +1 for spacing
                }
                return doc;
            },

            writeEquipmentString(doc, fontSize) {

                doc.setFontSize(fontSize);
                for (let i = 0, x =72, y = 222; i < this.rows.length ; i++){
                    doc.setFontType('bold');
                    doc.text("Device Type: ", x, y);
                    doc.setFontType('normal');
                    let labelLength = doc.getStringUnitWidth("Device: type: ") * fontSize;
                    doc.text(this.rows[i].device, labelLength + x +5, y);
	                y = y + 12; //increments y for spacing
	                doc.setFontType('bold');
                    doc.text("CMAC/SN: ", x, y);
                    doc.setFontType('normal');
                    let labelLength2 = doc.getStringUnitWidth("CMAC/SN: ")  * fontSize;
                    doc.text(this.rows[i].equipmentNum, labelLength2 + x + 5, y);
                    y = y + 12; //increments y for spacing
                    if (this.rows[i].powerCord || this.rows[i].remote){
                        doc.setFontType('bold');
                        doc.text("Accessories: ", x, y);
                        doc.setFontType('normal');
                        let labelLength3 = doc.getStringUnitWidth("Accessories: ") * fontSize;
                        if (this.rows[i].powerCord){
                            doc.text("Power Cord Included", labelLength3 + x + 5, y);
                        }
                        if (this.rows[i].remote){
                            y = y + 12;
	                        doc.text("Remote Included", labelLength3 + x + 5 ,y);
	                        y = y + 12;
                        }
                    }
                    y = y + 12
            ...
            writeReturnString(doc, fontSize) {
